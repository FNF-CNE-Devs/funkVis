...
class BarObject
{
	public var binLo:Int;
	public var binHi:Int;
	public var freqLo:Float;
	public var freqHi:Float;
	public var ratio:Float;
	public var ratioLo:Float;
	public var ratioHi:Float;
	public var recentValues:RecentPeakFinder;
}
...
class SpectralAnalyzer
{
	...
	function calcBars(barCount:Int, peakHold:Int)
	{
		...
		for (i in 0...barCount)
		{
			...
			var binLo = floorFreqToBin(freqLo);
			var binHi = roundFreqToBin(freqHi);
		...
		if (bars[0].freqLo < minFreq)
		{
			...
			bars[0].binLo = floorFreqToBin(minFreq);
		...
		if (bars[bars.length - 1].freqHi > maxFreq)
		{
			...
			bars[bars.length - 1].binHi = floorFreqToBin(maxFreq);
		...
		//     var binLo = floorFreqToBin(freqLo);
		//     var binHi = floorFreqToBin(freqHi);
	...
	inline function getBin(freq:Float):Float
	{
		return freq * fftN2 / audioClip.audioBuffer.sampleRate;
	}

	inline function floorFreqToBin(freq:Float):Int return Math.floor(getBin(freq));
	inline function roundFreqToBin(freq:Float):Int return Math.round(getBin(freq));
	inline function ceilFreqToBin(freq:Float):Int return Math.ceil(getBin(freq));

	inline function toDb(value:Float):Float
	{
		return 20 * LogHelper.log10(value);
	}

	inline function binToFreq(bin)
	...
	inline function average(input:Array<Float>):Float
	{
	...
	public static function applyEMASmoothing(previousSTFT:Array<Float>, updatedSTFT:Array<Float>, smoothingFactor:Float):Array<Float> {
		...
		smoothedSTFT.resize(updatedSTFT.length);
		...
		for (i in 0...updatedSTFT.length) {
			...
			smoothedSTFT[i] = Math.isNaN(smoothedValue) ? 0.0 : smoothedValue;
		}
